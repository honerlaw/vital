 ...
 * per-user state, the user's generated programs (030 — an active program may be a generated one, so
 * it must be merged before screens read `programs`), AND the equipment profile (042 — so the
 * routine intake pre-fills instantly). Error wins over loading so a failed fetch surfaces the Retry
 * view immediately instead of spinning forever behind another fetch.
 */
