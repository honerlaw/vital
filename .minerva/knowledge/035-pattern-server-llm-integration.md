# Pattern: server-side LLM integration — fetch-based Anthropic calls, server-only key, strict-writer guard validation
...
## Call Anthropic via `fetch`, not the SDK

The Messages API is reached through the platform `fetch` (`src/server/llm/call-claude.ts`), NOT
`@anthropic-ai/sdk`. No new dependency means no `package-lock` churn and no exposure to the
npm10/11 lockfile divergence trap (024) or EAS prune surprises. The model id is PINNED
(`claude-sonnet-4-6` in `model.ts`) — no unstable "latest" alias that could shift the
prompt/output contract under us. The client is reached only from `src/app/api/**` route handlers
(re-exported one-function-per-file from `src/server/routes/`), the same boundary as pg access (014).

## The key is server-only — NEVER `EXPO_PUBLIC_*`

`ANTHROPIC_API_KEY` is read from `process.env` inside `call-claude.ts` (exactly as `db.ts` reads
