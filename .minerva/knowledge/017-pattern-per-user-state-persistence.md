...
`user_state` (single row per user: `clerk_user_id` PK, `active_program_id`, `cursor`,
...
on a program removal); stale ids are normalized client-side at hydration. One global cursor
mirrors current client semantics; per-program cursors are an anticipated **additive** migration.

## Server contract (all `requireAuth`)

- `GET /api/me/state` → `{ activeProgramId, cursor, history }` in one bootstrap round-trip,
  **defaults-shape for a missing row** (`activeProgramId: null`, `cursor: 0`) — no 404
  special-case, so the client needs no absent-vs-error branch.
- `PUT /api/me/state` → parameterized single-row UPSERT of the settings pair.
- `POST /api/me/sessions` → session append + cursor upsert **atomically** (see the
  ...
  conflict arm updates only the cursor (finishing never changes the active program).
...
and persist-after-normalize PUTs the re-pointed id with the **preserved** cursor.
